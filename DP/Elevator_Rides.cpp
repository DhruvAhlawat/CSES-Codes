...
signed main()
{
    ...
    for (int k = 0; k < (1<<n); k++)
    {
        for (int i = 0; i < n; i++)
        {
            //the ith bit in k, if it is set then we have taken that
            if(((1<<i)&k))
            {
                pint stuff =dp[(k^(1<<i))] ;
                if(stuff.second + a[i] <= x)
                {
                    stuff.second += a[i];
                }
                else
                {
                    stuff.second = a[i];
                    stuff.first += 1;//just the ith element in the latest subset
                }
                dp[k] = min(stuff,dp[k]);
            }
        }

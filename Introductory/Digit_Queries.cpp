...
signed main()
{
    ...
    int t;
    cin >> t;
    //so we store the starts of all the digit changing numbers
    vector<int> starts(18,0);
    int tens = 1; 
    for (int i = 1; i < 18; i++)
    {
        starts[i] = starts[i-1] + 1 + (tens*9)*(i);
        tens *= 10;
    }
   printVectorArray(starts);
    for(int testCase = 0;testCase < t;testCase++)
    {
        int k;
        cin >> k;
        //1 indexed position in the string
        auto it = upper_bound(all(starts),k);
        it--;
        int index = it - starts.begin();
        
        //index + 1 is the number of digits the number it is in, has
        k -= *it;
        int num = (k+1)/(index+1);
    }

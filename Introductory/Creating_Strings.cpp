...
signed main()
{
    ...
    int prod = factorials[n];
    for (int i = 0; i < 26; i++)
    {
        prod /= factorials[alpha[i]]; //if they are 0 or 1, this is just 1 so no problemo
       //cout << "divided by " << factorials[alpha[s[i]]];
    }
    cout << prod << endl;
    //now to generate all possible strings
    //how do we do this

...
void moves(int n,int current,int target,int extra)
{
    if(n == 0)
        return;
    moves(n-1,current,extra,target);
    //then we move the current block to the place
    cout << current << " " << target << endl;
    moves(n-1,extra,target,current); //then we move the rest of the blocks on top of this block
}
signed main()
{
    FASTINOUT;

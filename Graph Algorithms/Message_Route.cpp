...
signed main()
{
    ...
    while(!check.empty())
    {
       ...
       // cout << "visited " << cur << endl;
        ...
        {
            if(!visited[Adjacent[cur][j]]) 
            {
                prev[Adjacent[cur][j]] = cur;
                check.push(Adjacent[cur][j]);
                visited[Adjacent[cur][j]] = true;
            }
        }
    }
    if(!visited[n-1])

...
bool DFS(int x, vector<vector<int>> &Adj,vector<int> &color)
{
    ...
    for (int i = 0; i < Adj[x].size(); i++)
    {
        ...
        if(color[Adj[x][i]] != 2)
        {
            continue;; //since already visited this node
        }
        //odd length cycle exists, so we return
        //else
        color[Adj[x][i]] = 1 - color[x]; //if color[x] = 1, then it is 0, and color[x] is 0, then it is 1
        CanDo = CanDo && DFS(Adj[x][i],Adj,color);
        if(!CanDo)
            return false;
    }
    return CanDo;
...
signed main()
{
    ...
    //gotta make the graph bipartite
    vector<int> color(n,2); //2 means non colored;
    //now we DFS each node
    bool CanDo = true;
    for (int i = 0; i < n; i++)
    {
        if(color[i] == 2)

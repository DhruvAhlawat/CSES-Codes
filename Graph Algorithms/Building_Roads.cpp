...
signed main()
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        cin >> x >> y;
        x--;y--;
        AdjacencyList[x].push_back(y); AdjacencyList[y].push_back(x);
    }
    vector<vector<int>> connectedComponents;
    //doing BFS for each vertex
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        while(!Check.empty())
        {
            ...
            if(visited[cur])
                continue;
            visited[cur] = true;
            newComponent.push_back(cur);
            //else we check its connected nodes
            for (int j = 0; j < AdjacencyList[cur].size(); j++)
            {
                //for each connected node
                if(visited[AdjacencyList[cur][j]])
                    continue;
                //else
                Check.push(AdjacencyList[cur][j]); //added new node to the queue if it is not visited before
            }
        }
    ...
    {
        cout << connectedComponents[i][0]+1 << " " << connectedComponents[i+1][0]+1 << endl;
    }

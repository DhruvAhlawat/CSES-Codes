...
signed main()
{
    ...
    int n,m;
    cin >> n >> m;
    //gotta find (n+m-1)C(m)
    vector<int> factorialModed(2000000+1,1);
    ...
    int ans = (factorialModed[n+m-1]*(binExpMod(factorialModed[m],MOD-2,MOD)))%MOD;
    ans *= binExpMod(factorialModed[n-1],MOD-2,MOD);
    ans %= MOD;
    cout << ans << endl;
}
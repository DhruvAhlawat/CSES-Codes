...
signed main()
{
    ...
    int powNum = 1;
    int totalPow = 0;
    vector<pint> primesAndPowers;
    for (int test = 0; test < n; test++)
    {
        int p,x;
        cin >> p >> x; totalPow += x;
        primesAndPowers.push_back({p,x});
        num *= x+1;
        powNum *= x+1;
        powNum %= MOD-1;
        num %= MOD;
        sum *= ((binExpMod(p,x+1,MOD)-1)*binExpMod(p - 1,MOD-2,MOD))%MOD;
        sum %= MOD;
        //prod *= binExpMod(p,(x*(x+1))/2,MOD);
        //prod %= MOD;
    }
    int pi = 1;
    for (int i = 0; i < n; i++)
    {
        int p = primesAndPowers[i].first, x = primesAndPowers[i].second;
        pi = (binExpMod(pi,x+1,MOD)*(binExpMod(binExpMod(p,((x)*(x+1))/2,MOD-1),powNum,MOD)))%MOD;
    }
    

    cout << num << " " << sum << " " << pi%MOD << endl;

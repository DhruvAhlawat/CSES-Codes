...
int nCr(int a,int b)
{
    ...
    {
        ans = (ans*(a-i))%MOD;
        ans = ans*binExpMod(i+1,MOD-2,MOD);
        ans %= MOD;
    }
    return ans;
}
signed main()
{
    ...
    //then calculate all factorials upto maxX
    vector<int> factorialModed(maxA+1,1);
    //vector<int> divisorFactorial(maxA+1,1);
    int cur = 1;
    for (int i = 1; i <= maxA; i++)
    {
        cur *= i;
        cur %= MOD;
        factorialModed[i] = cur;
    }
    cur = 1;
    // for (int i = 1; i < maxA+1; i++)
    // {
    //     cur *= binExpMod(i,MOD-2,MOD);
    //     cur %= MOD;
    //     divisorFactorial[i] = cur;
    // }
    for (int i = 0; i < n; i++)
    {
        int a = inputs[i].first,b = inputs[i].second;
        int ans = (factorialModed[a]*(binExpMod(factorialModed[b],MOD-2,MOD)))%MOD;
        ans *= binExpMod(factorialModed[a-b],MOD-2,MOD);
        ans %= MOD;
        cout << ans << endl;

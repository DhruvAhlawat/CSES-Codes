...
long long binExpMod(long long n,long long r,long long modulus)
{
    ...
    if(r == 1)
        return n;
    //if MOD is a prime, then uncomment the below line
    //r %= MOD-1;
    if(r%2 == 0)
    {
...
signed main()
{
    ...
    cin >> n;
    int ans = 0;
    ans += 2*binExpMod(2,(n*(n+1))/2,MOD);
    ans += binExpMod(2,n*n,MOD);
    if(n%2 == 0)
        ans += binExpMod(2,(n*n)/2,MOD);
    else
        ans += binExpMod(2,(n*((n+1)/2)),MOD);
    ans%= MOD;
    ans *= binExpMod(n,MOD-2,MOD);
    ans %= MOD;
    cout << ans << endl;

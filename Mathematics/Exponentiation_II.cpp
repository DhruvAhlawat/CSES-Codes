...
signed main()
{
    ...
    for(int testCase = 0;testCase < t;testCase++)
    {
        ...
        //not as simple as Exponentiation I
        //gotta find the correct mod for b^c so that we get the correct mod value for a^(b^c) MOD 10e9+7
        //so we know a^p is a mod p according to fermat's little theorem, gotta use this
        //also a^(p-1) is 1 mod p
        int k = binExpMod(b,c,MOD-1);
        //so know k is the remainder after taking all a^p-1 terms which have mod simply as 1
        cout << binExpMod(a,k,MOD) << endl;
    }

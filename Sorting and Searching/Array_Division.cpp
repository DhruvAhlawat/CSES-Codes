...
bool check(int SUM, const vector<int> &v,int k)
{
    int sum = 0; int partitions = 0;
...
signed main()
{
    ...
    //simple binary search again
    //since its much simpler to check if a solution is correct rather than finding a minimum solution
    int l = maxValue,r = n*maxValue;
    while(l < r)
    {
        int mid = (l+r)/2;
        if(check(mid,a,k))

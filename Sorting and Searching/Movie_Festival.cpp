...
signed main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        int a,b;
        cin >> a >> b;
        timings.push_back({b,a});
    }
    sort(timings.begin(),timings.end());
    // for (int i = 0; i < n; i++)
    // {
    //     cout << timings[i].first << " " << timings[i].second << endl;
    // }
    ...
    for (int i = 0; i < n; i++)
    {
        if(timings[i].second >= lastEnded)
        {
            count++;
            //cout << "watched the " <<i << "th movie cause " << timings[i].second << " >= " << lastEnded << endl;
            lastEnded = timings[i].first;
        }
    }

...
bool checkTime(int t,int products,const vector<int> &k)
{
    //now simply we check
    int a =0;
...
signed main()
{
    ...
    {
        cin >> k[i];
        minTime = min(k[i],minTime);
    }
    //can do binary search tho can't we, if we can do it in time t, then we can do it in time t+1
    //if we can make it in suppose c time
    //then we should have the inequality that c/t1 + c/t2 + c/t3 .... >= products
    int l = 1, r = (products*minTime);


    while(l < r)
    {
        int mid = (l+r)/2;
        if(checkTime(mid,products,k))
        {
            r = mid;
            continue;
        }

...
signed main()
{
    ...
    //n < 5000 therefore n^2 < 25*10^6, 2.5*10^7.
    //so n^2 seems possible
    sort(all(a));
    if(a[n-1].first + a[n-2].first + a[n-3].first < x)
    {
    ...
    for (int i = 1; i < n-1; i++)
    {
        //i is the location of the middle pointer
        l = 0,r = n-1;
        int sumToReach = x - a[i].first;
        while((l < i && r > i))
        {

...
signed main()
{
    ...
    //vector<int> maxSumTill(n,0);
    vector<int> maxSumEndingHere(n,0);
    maxSumEndingHere[0] = x[0];
    for (int i = 1; i < n; i++)
    {
        if(maxSumEndingHere[i-1] > 0)
            maxSumEndingHere[i] = maxSumEndingHere[i-1] + x[i];
        else
            maxSumEndingHere[i] = x[i];
        // if(maxSumEndingHere[i] > maxSumTill[i])
        // {
            
        // }
    }
    cout << *max_element(maxSumEndingHere.begin(),maxSumEndingHere.end()) << endl;

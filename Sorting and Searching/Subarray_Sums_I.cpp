...
signed main()
{
    ...
    int cnt = 0;
    int l = 1,r = 1;
    vector<int> pS(n+1,a[0]);
    pS[0] = 0;
    for (int i = 2; i <= n; i++)
    {
        pS[i] = pS[i-1] + a[i-1];
    }
    //so now we gotta calculate number of subarrays right.
    //since we know that all the numbers are positive, we can take advantage of that fact
    while(r <= n)
    {
        int sum = pS[r] - pS[l-1];
        if(sum > x)
        ...
        {
            cnt++; 
            //hence once we have sum = x, we must move l by one
            l++;
        }
    }
    cout << cnt << endl;
